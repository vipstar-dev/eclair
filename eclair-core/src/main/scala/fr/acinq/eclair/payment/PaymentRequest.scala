...
/**
  ...
  *
  * @param prefix    currency prefix; lnvips for vipstarcoin, lntvips for vipstarcoin testnet
...
object PaymentRequest {
  ...
  val prefixes = Map(
    Block.RegtestGenesisBlock.hash -> "lnrvips",
    Block.TestnetGenesisBlock.hash -> "lntvips",
    Block.LivenetGenesisBlock.hash -> "lnvips")
  ...
  object FallbackAddress {
    ...
    def toAddress(f: FallbackAddress, prefix: String): String = {
      import f.data
      f.version match {
        case 17 if prefix == "lnvips" => Base58Check.encode(Base58.Prefix.PubkeyAddress, data)
        case 18 if prefix == "lnvips" => Base58Check.encode(Base58.Prefix.ScriptAddress, data)
        case 17 if prefix == "lntvips" || prefix == "lnrvips" => Base58Check.encode(Base58.Prefix.PubkeyAddressTestnet, data)
        case 18 if prefix == "lntvips" || prefix == "lnrvips" => Base58Check.encode(Base58.Prefix.ScriptAddressTestnet, data)
        case version if prefix == "lnvips" => Bech32.encodeWitnessAddress("vips", version, data)
        case version if prefix == "lntvips" => Bech32.encodeWitnessAddress("tvips", version, data)
        case version if prefix == "lnrvips" => Bech32.encodeWitnessAddress("rvips", version, data)
      }
    }
  }
