...
class PaymentLifecycle(nodeParams: NodeParams, id: UUID, router: ActorRef, register: ActorRef) extends FSM[PaymentLifecycle.State, PaymentLifecycle.Data] {
  ...
  when(WAITING_FOR_PAYMENT_COMPLETE) {
    ...
    case Event(fulfill: UpdateFulfillHtlc, WaitingForComplete(s, c, cmd, _, _, _, _, hops)) =>
      ...
      context.system.eventStream.publish(PaymentSent(id, MilliSatoshi(c.paymentOptions.finalAmountMsat), MilliSatoshi(cmd.amountMsat - c.paymentOptions.finalAmountMsat), cmd.paymentHash, fulfill.paymentPreimage, fulfill.channelId))
      stop(FSM.Normal)
...
object PaymentLifecycle {
  ...
  // @formatter:off
  case class ReceivePayment(amountMsat_opt: Option[MilliSatoshi],
                            description: String,
  ...
  def buildPayloads(hops: Seq[Hop], opts: PaymentOptions): (Long, Long, Seq[OnionPerHopPayload]) = {
    val finalPayload: Seq[OnionPerHopPayload] = opts match {
      ...
      case p: TlvPayload => TlvStream[OnionTlv](OnionTlv.AmountToForward(p.finalAmountMsat) +: OnionTlv.OutgoingCltv(p.finalExpiry) +: p.records) :: Nil
    }
    hops.reverse.foldLeft((opts.finalAmountMsat, opts.finalExpiry, finalPayload)) {
  ...
  def buildCommand(id: UUID, paymentHash: ByteVector32, hops: Seq[Hop], opts: PaymentOptions): (CMD_ADD_HTLC, Seq[(ByteVector32, PublicKey)]) = {
    val (firstAmountMsat, firstExpiry, payloads) = buildPayloads(hops.drop(1), opts)
