...
trait Eclair {
  ...
  def receive(description: String, amountMsat_opt: Option[Long], expire_opt: Option[Long], fallbackAddress_opt: Option[String], paymentPreimage_opt: Option[ByteVector32], allowMultiPart_opt: Option[Boolean])(implicit timeout: Timeout): Future[PaymentRequest]
...
class EclairImpl(appKit: Kit) extends Eclair {
  ...
  override def receive(description: String, amountMsat_opt: Option[Long], expire_opt: Option[Long], fallbackAddress_opt: Option[String], paymentPreimage_opt: Option[ByteVector32], allowMultiPart_opt: Option[Boolean])(implicit timeout: Timeout): Future[PaymentRequest] = {
    ...
    (appKit.paymentHandler ? ReceivePayment(description = description, amountMsat_opt = amountMsat_opt.map(MilliSatoshi), expirySeconds_opt = expire_opt, fallbackAddress = fallbackAddress_opt, paymentPreimage = paymentPreimage_opt, allowMultiPart = allowMultiPart_opt.getOrElse(false))).mapTo[PaymentRequest]
  }
