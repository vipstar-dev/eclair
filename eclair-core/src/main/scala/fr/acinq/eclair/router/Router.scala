...
class Router(val nodeParams: NodeParams, watcher: ActorRef, initialized: Option[Promise[Done]] = None) extends FSMDiagnosticActorLogging[State, Data] {
...

  {
    ...
    startWith(NORMAL, Data(Map.empty, initChannels, None, Stash(Map.empty, Map.empty), rebroadcast = Rebroadcast(channels = Map.empty, updates = Map.empty, nodes = Map.empty), awaiting = Map.empty, privateChannels = Map.empty, excludedChannels = Set.empty, graph, sync = Map.empty))
  }
  ...
  def handle(u: ChannelUpdate, origin: ActorRef, d: Data, remoteNodeId_opt: Option[PublicKey] = None, transport_opt: Option[ActorRef] = None): Data = {
    // On Android, after checking the sig we remove as much data as possible to reduce RAM consumption
    val u1 = u // .copy(signature = null, chainHash = null)
    if (d.channels.contains(u.shortChannelId)) {
      ...
      if (d.rebroadcast.updates.contains(u)) {
        ...
        d.copy(channels = d.channels + (u.shortChannelId -> pc.updateChannelUpdateSameSideAs(u)), privateChannels = d.privateChannels - u.shortChannelId, rebroadcast = d.rebroadcast.copy(updates = d.rebroadcast.updates + (u1 -> Set(origin))), graph = graph1)
      }
