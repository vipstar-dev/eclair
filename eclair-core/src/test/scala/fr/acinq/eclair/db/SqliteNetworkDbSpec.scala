...
class SqliteNetworkDbSpec extends FunSuite {
  ...
  def shrink(c: ChannelAnnouncement) = c.copy(nodeSignature1 = null, nodeSignature2 = null, bitcoinKey1 = null, bitcoinKey2 = null, bitcoinSignature1 = null, bitcoinSignature2 = null, features = null, chainHash = null)
  ...
  def simpleTest(sqlite: Connection) = {
    ...
    // on Android we null as many fields as possible to save memory
    val channel_1s = shrink(channel_1)
    val channel_2s = shrink(channel_2)
    val channel_3s = shrink(channel_3)
    ...
    assert(db.listChannels() === SortedMap(
      channel_1.shortChannelId -> PublicChannel(channel_1s, txid_1, capacity, None, None),
      channel_2.shortChannelId -> PublicChannel(channel_2s, txid_2, capacity, None, None),
      channel_3.shortChannelId -> PublicChannel(channel_3s, txid_3, capacity, None, None)))
    ...
    assert(db.listChannels() === SortedMap(
      channel_1.shortChannelId -> PublicChannel(channel_1s, txid_1, capacity, Some(channel_update_1), Some(channel_update_2)),
      channel_3.shortChannelId -> PublicChannel(channel_3s, txid_3, capacity, Some(channel_update_3), None)))
