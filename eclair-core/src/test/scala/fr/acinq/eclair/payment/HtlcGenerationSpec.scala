...
import fr.acinq.eclair.{Globals, ShortChannelId, TestConstants, nodeFee, randomBytes32}
...
class HtlcGenerationSpec extends FunSuite with BeforeAndAfterAll {
  ...
  test("compute payloads with fees and expiry delta") {
    val (firstAmountMsat, firstExpiry, payloads) = buildPayloads(hops.drop(1), LegacyPayload(finalAmountMsat, finalCltvExpiry))
  ...
  test("build onion") {
    val (_, _, payloads) = buildPayloads(hops.drop(1), LegacyPayload(finalAmountMsat, finalCltvExpiry))
  ...
  test("build onion with final tlv payload") {
    val (_, _, payloads) = buildPayloads(hops.drop(1), TlvPayload(finalAmountMsat, finalCltvExpiry))
    val nodes = hops.map(_.nextNodeId)
    val PacketAndSecrets(packet_b, _) = buildOnion(nodes, payloads, paymentHash)
    assert(packet_b.payload.length === Sphinx.PaymentPacket.PayloadLength)
  ...
  test("build a command including the onion") {
    val (add, _) = buildCommand(UUID.randomUUID, paymentHash, hops, LegacyPayload(finalAmountMsat, finalCltvExpiry))
  ...
  test("build a command with no hops") {
    val (add, _) = buildCommand(UUID.randomUUID(), paymentHash, hops.take(1), LegacyPayload(finalAmountMsat, finalCltvExpiry))
...
object HtlcGenerationSpec {
  ...
  val finalCltvExpiry = Channel.MIN_CLTV_EXPIRY
  val currentBlockCount = 400000
  val finalExpiry = currentBlockCount + finalCltvExpiry + 1
